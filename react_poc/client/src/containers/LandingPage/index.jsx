...
class LandingPage extends Component {
    ...
    UpdateCenterFromChild(listing) {
        let coordinates = [];
        let lat1 = this.state.center.lat, lng1 = this.state.center.lng;
        let lat2 = listing.coordinates.lat, lng2 = listing.coordinates.lng;
        // If initial position is to the left of the destination position according to a +/- number line then true, else false.
        let incrementLat = lat1 < lat2 ? 0.1 : -0.1;
        let incrementLng = lng1 < lng2 ? 0.1 : -0.1;
        let updatedLat = lat1, updatedLng = lng1;
        console.log("lat1,lng1",lat1,lng1);
        console.log("lat2,lng2",lat2,lng2);
        while (updatedLat !== lat2 || updatedLng !== lng2) { // This while loop will increment/decrement the center until it reaches its desired destination.
            console.log("lmao",updatedLat,updatedLng);
            // updatedLat = ((updatedLat + incrementLat) < lat2) ? (updatedLat + incrementLat) : lat2;
            // updatedLng = ((updatedLng + incrementLng) < lng2) ? (updatedLng + incrementLng) : lng2;
            if (incrementLat) { // Increment
                updatedLat += 0.1;
                updatedLat = (updatedLat < lat2) ? updatedLat : lat2;
            } else {
                updatedLat -= 0.1;
                updatedLat = (updatedLat < lat2) ? updatedLat : lat2;
            }
            if (incrementLng) { // Increment
                updatedLng += 0.1;
                updatedLng = (updatedLng < lng2) ? updatedLng : lng2;
            } else {
                updatedLng -= 0.1;
                updatedLng = (updatedLng > lng2) ? updatedLng : lng2;
            }
            // coordinates.push({"lat":updatedLat,"lng":updatedLng});
        }
        ...
        //         }
        //     });
        //     console.log("HI",updatedLat,updatedLng);
        // }, 1000);
    }

    ToggleListingInfoFromChild(){
        this.setState({
        ...
        return (
            <Grid component="main">
                ...
                <Box className="App" component="div" display="flex" flexDirection="column">
                    <Grid container>
                        <Grid item xs={4}>
                            <ListView testList={testList} updateCenter={this.UpdateCenterFromChild} toggleListingInfo={this.ToggleListingInfoFromChild}/>
                        </Grid>
                        <Grid item>
                            <ListingInfo listing_info_opened={listing_info_opened} toggleListingInfo={this.ToggleListingInfoFromChild}/>
                        </Grid>
                        <Grid className="map" item xs={8}>
                            <MapView center={center} zoom={zoom} labelStyle={labelStyle} testList={testList} toggleListingInfo={this.ToggleListingInfoFromChild}/>
                        </Grid>
                    </Grid>
                </Box>
            </Grid>
